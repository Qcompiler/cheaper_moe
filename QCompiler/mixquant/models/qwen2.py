...
class Qwen2Fuser:
    ...
    def _fuse_qkv(self, module ,cache):
        ...
        if isinstance(qkv_layer, MixLinear_GEMM):
            ...
            qkv_layer.q_scale_col = torch.cat([q_proj.q_scale_col, k_proj.q_scale_col, v_proj.q_scale_col], dim=0)
...
class Qwen2MixQForCausalLM(BaseForCausalLM):
    ...
    def fuse_layers(model, quant_config: Dict, mix = False, cache = None):
        ...
        fuser = Qwen2Fuser(model, quant_config)
...
class Qwen2Fuser:
    def __init__(self, model, quant_config):
        ...
        self.rmsnorm_modules = [
            (name, module) for name, module in self.model.named_modules()
            if  "RMSNorm" in str(module.__class__)
        ]
    ...
    def fuse_mlp(self,mix, MixGemmCache = None):
        for name, module in self.mlp_modules:
            if  mix:
                assert MixGemmCache is not None
                mlp = MixQwen2MLP(module.gate_proj, module.down_proj, module.up_proj , MixGemmCache)
            set_module_name(self.model, name, mlp)