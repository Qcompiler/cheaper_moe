...
torch::Tensor FindRowScale(  const torch::Tensor &x,  torch::Tensor &scaleRow,
  ...
  if (bit == 8){
            ...
            auto quant_out = torch::zeros(
                ...
                dim3 block(256);
                dim3 grid(rows, 1);
            FindRowScaleKernel<256><<<grid, block,0, at::cuda::getCurrentCUDAStream().stream()>>>(
            ...
            auto quant_out = torch::zeros(
                ...
                dim3 block(256);
                dim3 grid(rows, 1);
            FindRowScaleKernel4bit<256><<<grid, block,0, at::cuda::getCurrentCUDAStream().stream()>>>(
...
void int4FusedDequantizeCUDA_(const cutlass::int4b_t *A,
...
void int8quant_(int rows, int cols, const half * src, int8_t *output, 
    ...
    dim3 block(256);
    dim3 grid(rows, 1);
    FindRowScaleKernel_<256><<<grid, block, 0, stream>>>(
                output,
                src, scale,
                rows, cols);
...
void int4quant_(int rows, int cols, int colsDst, const half * src, Int4Storage *output, 
        half *scale, cudaStream_t stream){
    ...
    dim3 block(256);
    dim3 grid(rows, 1);
    FindRowScaleKernel4bit<256><<<grid, block, 0, stream>>>(
        output,
        src, scale,
        rows, cols, colsDst);
...
torch::Tensor mixgemmforward(int M, int N, int K, 
                            torch::Tensor & A_, torch::Tensor &w_, torch::Tensor &s_,
                            torch::Tensor &fp_w_, torch::Tensor & ind_, torch::Tensor & q_w_, 
                            torch::Tensor & q_s_){
...
torch::Tensor mixgemmforward4bit(int M, int N, int K, 
    ...
    auto options_i4 = torch::TensorOptions().dtype
            (TorchDtypeDispatcher<Int4Storage>::value).device(A_.device());
    unsigned colsDst = (K - 1) / 2 + 1;
            assert (colsDst *2 == K);
    auto quant_out = torch::zeros(
      {M, colsDst }, options_i4);

    Int4Storage* int4_out = reinterpret_cast<Int4Storage *>(quant_out.data_ptr<Int4Storage>());
    ...
    const uint8_t * W = reinterpret_cast<uint8_t const*>(w_.data_ptr<uint8_t>());
    ...
    cublasSetStream(handle, stream);
    gemmfp16_(fp_activation,fp_weight,Out, M, N, num_ind, handle, stream);
    int4quant_(M, K, colsDst, A, int4_out, scale_a, stream);



    int4FusedDequantizeCUDA_((const cutlass::int4b_t *)quant_out.data_ptr<uint8_t>(),
                             (const cutlass::int4b_t * )W, 
                            scale_a,
                            scale_b, Out, Out, M, N, K, 
                            reinterpret_cast<char*>(int4_out),
                            stream);
    return fp16_out;
}
